...
/**
 * Methods useful for implementing the game Wordle. None of the methods assume
 * that the guess or answer words have exactly 5 letters.
 */
public class WordleUtils {
	...
	public static ArrayList<Boolean> isGreen(String guess, String answer) {
		...
		if (guess.length() != answer.length()) {
			throw new IllegalArgumentException("Guess not same length as answer");
		}
	...
	public static ArrayList<Boolean> isYellow(String guess, String answer) {
		...
		for (int i = 0; i < answer.length(); i++)// loop through each word
		{
			boolean tempGreen = green.get(i);

			if (tempGreen == true) {

				answer = (replace(answer, i, '!'));// all green letters are gone

			}

		}

		for (int j = 0; j < guess.length(); j++)// loop through each letter
		{

			if (answer.contains(String.valueOf(guess.charAt(j))))// if it contains letter at different position
			{

				for (int k = 0; k < guess.length(); k++)// loop through each letter until letter is found 
				{

					if (guess.charAt(j) == answer.charAt(k)) {
						ans.add(true);
						answer = (replace(answer, k, '!'));
						break;
				...
				ans.add(false);//false if letter not found in answer
			}
		}
	...
	public static ArrayList<WordleColor> getColors(String guess, String answer) {
		...
		WordleColor c = WordleColor.GREEN;
		WordleColor y = WordleColor.YELLOW;
		WordleColor g = WordleColor.GRAY;
		...
		for (int i = 0; i < guess.length(); i++) {
			boolean tempGreen = green.get(i);
			boolean tempYellow = yellow.get(i);
	 ...
	 * @throws IllegalArgumentException if guess and answer have different lengths
	 */
	public static void updateLetters(String guess, String answer, TreeSet<Character> included,
			TreeSet<Character> excluded, TreeSet<Character> possible) {
